...
class TrainListController extends Controller
{
    ...
    public function index(Request $request)
    {
    	...
    	$ob = \DB::select("select * from train where startStation like '%{$startCity}%' and arriveStation like '%{$endCity}%'");

    	return view("home.trainList.index",['ob'=>$ob,'startCity'=>$startCity,'endCity'=>$endCity]);
    ...
    public function show(Request $request)
    {
    	...
    	if($request->has('backDate')==false){
    		...
    		$ob = \DB::table("train")-> where("startStation","like","%{$startCity}%")
            ->where("arriveStation","like","%{$endCity}%")->paginate(5);
            //dd($where);
           //dd($ob); 
    		return view("home.trainList.index",['ob'=>$ob,'where'=>$where,'startCity'=>$startCity,'endCity'=>$endCity]);
