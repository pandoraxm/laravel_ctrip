            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title"><i class="fa fa-th"></i> 管理员信列表</h3>
                  &nbsp; <button class="btn btn-sm btn-primary" onclick="window.location='{{URL('admin/users/create')}}'">添加用户</button
                </div><!-- /.box-header -->
                <div class="box-body">
                    ...
                    @foreach($list as $vo)
                        <tr>
                            ...
                            <td><button class="btn btn-xs btn-danger" onclick="doDel({{ $vo->id }})">删除</button> 
                               <button class="btn btn-xs btn-success" onclick="loadRole({{ $vo->id}},'{{ $vo->name}}')">授权</button></td>
                        </tr>
                    @endforeach
